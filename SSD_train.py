...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (images, loc_targets, conf_targets,real_boxes,real_class) in enumerate(testloader):
            ...
            #     fin_bboxes[i],fin_class[i],score = data_encoder.decode(loc_preds[i].data.cpu(), 
            #                 F.softmax(conf_preds[i],dim=1).data.cpu(),threshold=0.1)#
            # class_acc,IoU = compute_iou_acc(fin_class.to(device).long(),real_class+1,fin_bboxes.to(device),real_boxes)
            # acc += class_acc
            # mean_iou += IoU
        print('\nAverage loss/batch: %.3f Mean IoU: %.3f Classificatin Acc: %.3f' 
                %(test_loss/(batch_idx+1),mean_iou/total,1.*acc/total) )
